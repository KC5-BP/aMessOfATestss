...
Client::Client(QWidget *parent) : QDialog(parent) {
    ...
    connect(connectBtn, &QPushButton::clicked, this, &Client::toggleConnection);
...
void Client::readSrvResponse(void) {
    ...
    if (streamAsString.toLower() == QString("leaving"))
        toggleConnection(); /* A bit dirty, but for a test, it does the trick */
...
void Client::toggleConnection(void) {
    ipLinEdit->setEnabled(socketConnectStatus);
    portLinEdit->setEnabled(socketConnectStatus);
